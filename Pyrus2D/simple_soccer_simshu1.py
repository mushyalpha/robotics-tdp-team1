...
DEFAULT_MATCH_DURATION = 300.0        # seconds (5 minutes)

# --- BASELINE behaviour parameters (from behavior_profile.py) ---
BASELINE_KICK_POWER    = 1.8
BASELINE_KICK_ACCURACY = 0.15
BASELINE_PASS_POWER    = 1.2
BASELINE_SHOOT_DIST    = PITCH_LENGTH * 0.55
...
class Soccer4v4:
    ...
    def __init__(self, mode: str = 'dynamic'):
        ...
        self.blue_goals = 0
        self.red_goals  = 0
        self.time       = 0          # steps elapsed this match
        self.match_duration        = DEFAULT_MATCH_DURATION
        self.match_duration_steps  = int(DEFAULT_MATCH_DURATION / DT)
        self.log        = []
    ...
    def reset_after_goal(self, next_kickoff_team: str):
        """Called when a goal is scored. Resets positions for the next kickoff
        within the same match (duration continues running)."""
        self._blue_last_formation = None
        self._red_last_formation  = None
        self.kickoff_team         = next_kickoff_team
        self.phase                = 'kickoff_setup'
        self.kickoff_wait         = 0
        self.ball.reset()
    ...
    def _act_with_ball(self, p: Player, own_team: list, attack_dir: int):
        ...
        if d_goal < BASELINE_SHOOT_DIST:
            ...
            b.kick(p, enemy_goal_x, 0.0,
                   power=BASELINE_KICK_POWER + np.random.uniform(-0.2, 0.2),
                   accuracy=BASELINE_KICK_ACCURACY)
            p.state = 'shoot'
        else:
            ...
            if best is not None:
                b.kick(p, best.x, best.y,
                       power=BASELINE_PASS_POWER + p.dist_to(best) * 0.14,
                       accuracy=BASELINE_KICK_ACCURACY)
                p.state = 'passing'
            else:
                b.kick(p, enemy_goal_x, b.y,
                       power=BASELINE_KICK_POWER, accuracy=BASELINE_KICK_ACCURACY)
                p.state = 'kick_fwd'
...
class Visualiser:
    def __init__(self, sim: Soccer4v4, match_duration: float = DEFAULT_MATCH_DURATION):
        self.sim             = sim
        sim.match_duration        = match_duration
        sim.match_duration_steps  = int(match_duration / DT)
    ...
    def update(self, frame):
        ...
        # ── End-of-match detection (time only -- goals now restart kickoff in-match) ──
        if sim.time >= sim.match_duration_steps:
            if not self._stats_printed:
                self._stats_printed = True
                sim.record_match_result()
                elapsed_s = sim.time * DT
                mm, ss = divmod(int(elapsed_s), 60)
                dur_mm, dur_ss = divmod(int(sim.match_duration), 60)
                print(f"\n[FULL TIME]  Blue {sim.blue_goals} - {sim.red_goals} Red  "
                      f"[{mm:02d}:{ss:02d} / {dur_mm:02d}:{dur_ss:02d}]")
        ...
        elapsed_s  = sim.time * DT
        total_s    = sim.match_duration
        e_mm, e_ss = divmod(int(elapsed_s), 60)
        t_mm, t_ss = divmod(int(total_s), 60)

        hud = (f"  Blue {sim.blue_goals}  :  {sim.red_goals} Red  |  "
               f"{e_mm:02d}:{e_ss:02d} / {t_mm:02d}:{t_ss:02d}  |  {phase_txt}  ")
    ...
    def run(self):
        def on_close(event):
            if self._stats_printed:
                return
            self._stats_printed = True
            # Window closed early before full time
            sim = self.sim
            sim.record_match_result()
            elapsed_s = sim.time * DT
            mm, ss = divmod(int(elapsed_s), 60)
            dur_mm, dur_ss = divmod(int(sim.match_duration), 60)
            print(f"\n[ABANDONED]  Blue {sim.blue_goals} - {sim.red_goals} Red  "
                  f"[{mm:02d}:{ss:02d} / {dur_mm:02d}:{dur_ss:02d}]")
            print_final_stats(sim)
...
def print_final_stats(sim: Soccer4v4):
    bs  = sim.blue_stats
    rs  = sim.red_stats
    sep = "=" * 62
    ...
    print(f"  Duration: {int(sim.match_duration // 60):02d}:{int(sim.match_duration % 60):02d}")
    print(sep)
    print(f"  {'Metric':<32} {'Blue':>12} {'Red':>12}")
    print(f"  {'-'*32} {'-'*12} {'-'*12}")
    print(f"  {'Goals scored':<32} {sim.blue_goals:>12} {sim.red_goals:>12}")
    print(f"  {'Shots taken':<32} {bs.shots:>12} {rs.shots:>12}")
    print(f"  {'Shots conceded':<32} {bs.shots_conceded:>12} {rs.shots_conceded:>12}")
    if bs.shots > 0 or rs.shots > 0:
        print(f"  {'Shot accuracy (%)':<32} "
              f"{sim.blue_goals/bs.shots*100 if bs.shots else 0:>11.1f}% "
              f"{sim.red_goals/rs.shots*100 if rs.shots else 0:>11.1f}%")
    if sim.mode == 'dynamic':
...
def select_mode() -> tuple:
    ...
    print()
    while True:
        raw = input("  Enter match duration in seconds (Enter = default 300): ").strip()
        if raw == '':
            match_duration = DEFAULT_MATCH_DURATION
            break
        try:
            match_duration = float(raw)
            if match_duration > 0:
                break
    ...
    mm, ss = divmod(int(match_duration), 60)

    print()
    print(f"  Mode selected  : {MODE_NAMES[mode]}")
    print(f"  Match duration : {mm:02d}:{ss:02d}  ({int(match_duration)}s, "
          f"{int(match_duration / DT)} steps @ DT={DT}s)")
    print()
...
if __name__ == '__main__':
    mode, match_duration = select_mode()
    sim = Soccer4v4(mode=mode)
    viz = Visualiser(sim, match_duration=match_duration)
    viz.run()